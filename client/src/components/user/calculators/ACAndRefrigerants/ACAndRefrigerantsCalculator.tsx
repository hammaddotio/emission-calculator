...
const RefrigerantEmissionCalculator: React.FC = () => {
    ...
    const callApi = async () => {
        try {
            ...
            setRefrigerants([]);  // Assuming setRefrigerants is the function to reset refrigerants
        } catch (error: string | any) {
            console.log(error.response?.data?.error ? error.response.data.error : error.message);
            message.error(error.response?.data?.error?.[0] || "An error occurred");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="p-6 mx-auto max-w-full bg-white rounded-lg shadow-lg border border-gray-300">
            ...
            {/* Wrap the table with overflow-x-auto for mobile screens */}
